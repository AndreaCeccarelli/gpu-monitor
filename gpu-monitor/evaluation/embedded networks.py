...
FILENAME='cifar10_COMPLETE_DATASET.csv'
EPOCHS=500 #tanto c'è lo stop dopo 3 non-improvements
LAYER1=[1000, 600, 400, 300, 200, 100] #layer size
LAYER2=[20, 100, 200, 400, 600, 800] #layer size
ATTACKLIST=['FastGradientMethod', 'BasicIterativeMethod', 'HopSkipJump', 'DeepFool', 'NewtonFool','ProjectedGradientDescentPyTorch','ElasticNet', 'Zoo', 'allblack']
#  'FULL', 'AdversarialPatchNumpy', 'SpatialTransformation', 'CarliniL2Method', 'SimBA', 'SquareAttack', 'CarliniLInfMethod', 'allwhite', 
#  
# 
...
LOGFILE='/home/andrea/gpu-monitor/logs/cifar10-full.log'
DIR='/home/andrea/gpu-monitor/dataset/FULL/'
PATH=DIR+FILENAME
...
# FILENAME='mnist_COMPLETE_DATASET.csv'
#L1, L2= 10, 50 , ACCURAY: 0.72
#L1, L2= 50, 100 , ACCURAY: 0.71
...
alwaysdrop=['timestamp', 'accounting.buffer_size','fan.speed','pstate',
...
datasetContinuous=['power.draw',
                   'mem_clock_avg',
                   'mem_clock_std',
                   'graphics_clock_avg',
                   'graphics_clock_std'] #5
...
for attack in ATTACKLIST:
    ...
    for layer1 in LAYER1:
        for layer2 in LAYER2:
            ...
            history=model.fit(input_list,y_train,batch_size=24576,epochs=EPOCHS,
                              validation_split=TRAIN_VAL_SPLIT, shuffle=True,
                              callbacks=[callback], verbose=1)
            print("Length of training in # of epoch: "+str(len(history.history['loss'])))

            print("EVALUATING ON TRAIN")
            score=model.evaluate(input_list,y_train, batch_size=24576, verbose=1)
            print(str(model.metrics_names))
            print(str(score))
            print("EVALUATE ON TEST")
            score=model.evaluate(test_list,y_test, batch_size=24576, verbose=1)
            ...
            file.write(", ACCURACY : "+str(score[1]))
            file.write(", tp : "+str(score[2]))
            file.write(", fp : "+str(score[3]))
            file.write(", tn : "+str(score[4]))
            file.write(", fn : "+str(score[5]))
            file.write("\n")
            file.flush()
            file.close()
